...
struct RegistrationPromptView: View {
    ...
    @EnvironmentObject var localizationManager: LocalizationManager  // ДОДАЙТЕ
    ...
    var body: some View {
        ZStack {
            // Градієнтний фон з підтримкою темної теми
            Group {
                    ...
                    Button {
                        ...
                        HStack(spacing: 12) {
                            Image(systemName: "envelope.fill")
                                .font(.system(size: 18))
                            Text(localizationManager.currentLanguage == .ukrainian ? "Продовжити з Email" :
                                 localizationManager.currentLanguage == .polish ? "Kontynuuj z Email" :
                                 "Continue with Email")
                    ...
                    Button {
                        ...
                        VStack(spacing: 4) {
                            Text(localizationManager.currentLanguage == .ukrainian ? "Продовжити без реєстрації" :
                                 localizationManager.currentLanguage == .polish ? "Kontynuuj bez rejestracji" :
        ...
        .sheet(isPresented: $showEmailRegistration) {
            EmailRegistrationPromptView(onComplete: onComplete)
